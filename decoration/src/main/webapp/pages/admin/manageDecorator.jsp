...
<div class="wrapper">
                        ...
                        <div class="box-header">
                            <div class="row">
                                <form class="form-horizontal" id="condition">
                                    ...
                                    <div class="col-xs-2">
                                        <button type="button" class="btn btn-block btn-primary" id="queryDecorator">查询
                                        </button>
                                    </div>
                                </form>
                            </div>
    ...
    <%--删除账户--%>
    <div class="modal fade modal-danger" id="deleteDecoratorDialog">
    ...
    <script>
        $(document).ready(function () {
            ...
            function Bootstrap_paging(page) {
                ...
                $.ajax({
                    type: "GET",
                    url: "<c:url value='/decorator/queryDecorators'/>",
                    dataType: "json",
                    data: json,
                    success: function (data) {
                        ...
                        for (var i = 0; i < data.result.size; i++) {
                            ...
                            switch (data.result.data[i].status) {
                                case("0"):
                                    decoratorStatus = "<span class='label label-success' >未接单</span>";
                                    break;
                                case("1"):
                                    decoratorStatus = "<span class='label label-success'>已接单</span>";
                                    break;
                            }
                            var node = "<tr>" +
                                "                    <td>" + ((page - 1) * 10 + i + 1) + "</td>" +
                                "                    <td>" + (data.result.data[i].cid == null ? "无" : data.result.data[i].cid) + "</td>" +
                                ...
                                "                    <td>" + decoratorStatus + "</td>" +
                                "                    <td>" ;
                            node+="<button type='button' decoratorId=" + data.result.data[i].id + " class='btn btn-danger deleteDecorator'>删除</button></td>";
                            $("#tableContent").append(node);
                        }

                        $(".deleteDecorator").click(function () {
                            $("#deleteDecoratorDialog").modal('show');
                            $("#deleteForm input[name='id']").val($(this).attr("decoratorId"));
                        })
                        var options = {
            ...
            $("#confirmDeleteBtn").click(function(){
                ...
                $.ajax({
                    type: "post",
                    url: "<c:url value='/decorator/updateDecorator'/>",
                    dataType: "json",
                    data: json,
                    success: function (data) {
                        ...
                        $("#deleteDecoratorDialog").modal('hide');
                    }
                })
            })
        })
